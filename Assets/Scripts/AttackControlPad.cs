...
public class AttackControlPad : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
        ...
        if (State == StateEnum.InvalidDragging || State == StateEnum.ValidDragging)
        {
            ...
            if (dragMagnitude > MainController.Instance.DragThreshold)//有效拖动
            {
                ...
                if (State == StateEnum.InvalidDragging)
                {
                    ...
                    MainController.Instance.Arrow2.gameObject.SetActive(true);
                    MainController.Instance.Arrow2.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, 0);
                ...
                var validDragDisplacement = CalcValidDragDisplacement(unit, CurrentAttackSkillID, dragDisplacement);
                ...
                unit.UnitInfoCanvas.SprCostMP.anchorMin =
                    unit.UnitInfoCanvas.SprCostMP.anchorMin.SetV2X(1 - unit.GetSkillCostMP(CurrentAttackSkillID, geodesicDisplacement.magnitude) / (unit.Data.mp + float.Epsilon));

                //辅助瞄准
                _assistAimingTarget = null;
                if (MainController.Instance.SkillAssistAimingList[CurrentAttackSkillID])
                {
                ...
                var arrow = MainController.Instance.Arrow2;
                ...
                arrow.position = unit.transform.position.SetV3Y(MainController.Instance.SkillArrowHeightList[CurrentAttackSkillID]);
                ...
                arrow.GetComponent<Image>().color = unit.GetSkillTotalCDRemaining(CurrentAttackSkillID) <= 0 && unit.GetSkillCountEnough(CurrentAttackSkillID) ? new Color(1, 0.5f, 0.5f) : Color.grey;
                ...
                if (_assistAimingTarget) //找到辅助瞄准的目标了
                {
                    ...
                    thinArrow.position =
                        unit.transform.position.SetV3Y(MainController.Instance.SkillArrowHeightList[CurrentAttackSkillID]);
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        if (State == StateEnum.ValidDragging)
        {
            ...
            if (dragMagnitude > MainController.Instance.DragThreshold) //有效拖动
            {
                ...
                if (MainController.Instance.SkillAssistAimingList[CurrentAttackSkillID] && _assistAimingTarget)
                {
                ...
                MainController.Instance.FocusedUnit.CastSkill(CurrentAttackSkillID, castSkillDisplacement);
            }
        }
    ...
    Vector3 DragDisplacementToGeodesicDisplacement(Vector2 dragDisplacement)
    {
        ...
        var geodesicDisplacement = (endPos - startPos).normalized*dragDisplacement.magnitude*
                                   ...
                                   MainController.Instance.FocusedUnit.SkillDragToDisplacementRatioList[CurrentAttackSkillID];
    ...
    Vector3 CalcValidDragDisplacement(Unit unit, int skillID, Vector2 dragDisplacement)
    {
        var dragMagnitude = dragDisplacement.magnitude;
        ...
        validDragMagnitude = Mathf.Clamp(validDragMagnitude, 0,
            MainController.Instance.SkillDragMaxLimitList[skillID]);
        validDragMagnitude = Mathf.Clamp(validDragMagnitude, 0, unit.GetSkillMaxGeodesicDistance(skillID) / MainController.Instance.FocusedUnit.SkillDragToDisplacementRatioList[skillID]);
